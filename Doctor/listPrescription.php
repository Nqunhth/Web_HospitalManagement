...
require "../Models/Prescription/Prescription.php";


$count = Prescription::fetchCountTotal();
if ($count->num_rows > 0) {
...
$result = Prescription::fetchPresPage($start, $limit);

if (isset($_POST['switch-change'])) {
    if (isset($_POST['pres-id'])) {
        if ($_POST['switch-change'] == "enable") {
            Prescription::disableForm($_POST['pres-id']);
        }
        if ($_POST['switch-change'] == "disable") {
            Prescription::enableForm($_POST['pres-id']);
        }
    }
}
...
<body>
        ...
        <ul class="navbar--list">
            ...
            <?php if (!empty($_SESSION['position'])) : ?>
                <li class="navbar--item has-dropdown-menu">
                    <a href="/Web_HospitalManagement/Doctor/listPrescription.php" class="navbar--item-link  is-active-in-navbar">Workspace</a>
                </li>
            <?php endif ?>
                            ...
                            <li class="card-drop">
                                ...
                                <div class="short-card">
                                    <div class="inner-card">
                                        ...
                                        <form class="switch-container center" method="post" action="">
                                            ...
                                            <label class="switch">
                                                <?php
                                                if (isset($_POST['switch-change']) && $_POST['pres-id'] == $row["pres_id"]) {
                                                    if ($_POST['switch-change'] == "disable") {
                                                ?>
                                                        <input class="" name="enable" value="<?php echo $row["pres_id"] ?>" type="checkbox" checked onclick="return showBox(event);">
                                                        <span class="slider round"></span>
                                                    <?php } else { ?>
                                                        <input class="" name="disable" value="<?php echo $row["pres_id"] ?>" type="checkbox" onclick="return showBox(event);">
                                                        <span class="slider round"></span>
                                                    <?php }
                                                } else {
                                                    if ($row["pres_status"] == 'enabled') { ?>
                                                        <input class="" name="enable" value="<?php echo $row["pres_id"] ?>" type="checkbox" checked onclick="return showBox(event);">
                                                        <span class="slider round"></span>
                                                    <?php } else { ?>
                                                        <input class="" name="disable" value="<?php echo $row["pres_id"] ?>" type="checkbox" onclick="return showBox(event);">
                                ...
                                <div class="full-card">
                                    <div class="inner-card">
                                        ...
                                        <div class="icon-container center hide">
        ...
        <div class="modal__overlay center">
            <div class="confirm-alert flex-column">
                ...
                <form class="alert-opts center" method="post" action="">
                    <input name="pres-id" class="hide pres-id" type="text">
    ...
    <script type="text/javascript">
        ...
        const id = document.querySelector('.pres-id');
